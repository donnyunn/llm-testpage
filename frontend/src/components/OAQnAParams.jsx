// src/components/OAQnAParams.jsx
import React, { useState, useEffect } from 'react';

const OAQnAParamsView = ({ fastapiBaseUrl }) => {
  // Text-to-SQLParamsView와 유사한 상태 변수들
  const [modelId, setModelId] = useState('google/gemma-2b-it'); // OA Q&A에 적합한 초기 모델 (선택 사항)
  // ★★★ 시스템 메시지 기본값 변경 (schema 없음) ★★★
  const [systemMessage, setSystemMessage] = useState(`You are an IT support assistant. Your task is to provide concise and helpful answers to user questions. Respond in Korean.`);
  ...
  // ★★★ tableColumns 변경 (schema 없음) ★★★
  const tableColumns = ['question', 'answer']; 
  const FILE_TYPE = "oa-qna";
  // 'id'는 백엔드와 내부적으로 관리되므로 UI 표시 컬럼에서는 제외했습니다.
  
  // 학습 파라미터 상태 변수들 (Text-to-SQLParamsView와 동일하게 유지)
  ...
  // 데이터 불러오기 함수 (FastAPI /data-entries 호출) - TextToSQLParamsView와 동일
  ...
  // 파일 핸들러 (Excel 파일 업로드) - TextToSQLParamsView와 동일
  ...
  const handleFileUpload = async () => {
    ...
    try {
      const response = await fetch(`${fastapiBaseUrl}/upload-oa-qna-data`, {
        method: 'POST',
        body: formData,
      });
  ...
  // 학습 시작 핸들러 - TextToSQLParamsView와 동일
  // 학습 파라미터에 system_message 포함
  const handleStartTraining = async () => {
    ...
    const trainingParams = {
        model_id: modelId,
        system_message: systemMessage, // OA Q&A용 시스템 메시지
  ...
  // 데이터 편집/저장/삭제 함수들 - TextToSQLParamsView와 동일
  ...
  const handleSave = async (index) => {
    ...
    // OA Q&A는 schema가 없으므로 requestBody에서 schema를 제외합니다.
    const requestBody = isNew ? { question: entryToSave.question, answer: entryToSave.answer } : entryToSave;

    try {
      const response = await fetch(isNew ? `${fastapiBaseUrl}/add-data/${FILE_TYPE}` : `${fastapiBaseUrl}/update-data/${FILE_TYPE}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  // 새 데이터 추가 함수 (schema 없음)
  const handleAddEntry = () => {
    setDataEntries([...dataEntries, { id: null, question: '', answer: '' }]); // schema 필드 제거
  ...
  return (
    <div className="learning-params-section">
      ...
      {/* 3. 학습 데이터 업로드 섹션 (schema 언급 제거) */}
      <h3>3. 학습 데이터 업로드</h3>
      <p>학습에 사용할 .xlsx 파일을 업로드하세요. (헤더: id, question, answer)</p> {/* schema 헤더 언급 제거 */}
      <input
        type="file"
        accept=".xlsx"
        ...
        <div style={{ maxHeight: '1000px', overflowY: 'auto', overflowX: 'auto', border: '1px solid #ccc', borderRadius: '5px' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ backgroundColor: '#f2f2f2' }}>
              <tr>
                {/* tableColumns는 question, answer만 포함하므로 schema 헤더는 자동으로 제외됩니다. */}
            ...
            <tbody>
              {dataEntries.map((entry, index) => (
                <tr key={typeof entry.id === 'number' ? entry.id : `new-${index}`}>
                  {/* tableColumns는 question, answer만 포함하므로 schema 셀은 자동으로 제외됩니다. */}
                  {tableColumns.map((key) => (
      ...
      {/* 5. 성능 최적화 설정 섹션 --- 6. PEFT (LoRA) 설정 섹션 --- 7. 학습 인자 섹션 */}
      {/* 이 부분들은 TextToSQLParamsView와 동일하게 유지 */}
      <h3>5. 성능 최적화 설정</h3>
